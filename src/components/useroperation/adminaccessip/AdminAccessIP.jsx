...
import AddIpBan,{DeletIpBan} from '../ipbanlist/AddIpBan';

export default function AdminAccessIP({title}){
    ...
    const fetchingData = async () => {
        try {
            ...
            const response = await api.get('/adminaccessips', {
    ...
    //  
    function updateDate(datas){
        console.log(datas);
        let showData = datas.map((item, index) => ({
            key: item.id,
            no: index + 1,
            id: item.id,
            ipaddress: item.ip,
