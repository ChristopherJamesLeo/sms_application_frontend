...
import StageExport from '../export/StageExport';
...
export default function Stages({title}){
    ...
    const fetchingData = async () => {
        try {
            // Initialize Pusher only once when the component mounts
            const pusher = new Pusher('a12455a52128baa242fd', {
                cluster: 'ap1',
            });
    
            // Subscribe to the channel and event
            const channel = pusher.subscribe('stage-channel');
            channel.bind('stage-event', (data) => {
                // console.log("Received from Pusher:", data.stages);
                updateStageData(data.stages);
            ...
            <div className="table-container">
                ...
                <div className="my-4 ">
                    ...
                    <div className='flex justify-end'>
                        {/* <UserSearch/> */}
                        <StageExport statuses={statuses} updateStageData={updateStageData}/>
                    </div>
                </div>
                <Table
                    dataSource={showData}
