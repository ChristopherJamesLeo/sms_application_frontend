...
import AddCourseType,{EditCourseType} from '../models/SettingModels/CourseType';
...
export default function CourseTypes({title}){
    ...
    const onChange = async (checked, idx) => {
        ...
        try {
            const response = await api.put(`/coursetypes/status/${idx}`, values, {
    ...
    const fetchingData = async () => {
        try {
            const response = await api.get('/coursetypes', {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('api_token')}` }
            });
        ...
        <div className="table-container">
            ...
            <div className="my-4 ">
                <div className=' mb-3 flex gap-x-2'>
                    <AddCourseType fetchData = {fetchingData} />
                </div>
