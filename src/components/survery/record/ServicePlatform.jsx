...
const AddServicePlatform = ({ setfetchData, fetchData }) => {
    ...
    const formHandler = async (values) => {
        try {
            const response = await api.post('/serviceplatforms', values, {
...
export function EditServicePlatform({ idx, name, fetchData }) {
    const [open, setOpen] = useState(false);
    const [form] = Form.useForm();
    const [messageApi, contextHolder] = message.useMessage();
    ...
    const formHandler = async (values) => {
        values.id = idx;
        try {
            const response = await api.put(`/serviceplatforms/${idx}`, values, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('api_token')}` }
            });
            if (response.data) {
                if (response.data) {
...
export default AddServicePlatform;
