...
const AddStatus = ({ setfetchData, fetchData }) => {
    ...
    const formHandler = async (values) => {
        try {
            const response = await api.post('/prefixes', values, {
...
export function EditPrefix({ idx, name, prefix ,fetchData }) {
    const [open, setOpen] = useState(false);
    const [form] = Form.useForm();
    const [messageApi, contextHolder] = message.useMessage();
    ...
    const formHandler = async (values) => {
        try {
            const response = await api.put(`/prefixes/${idx}`, values, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('api_token')}` }
            });
            if (response.data) {
                if (response.data) {
        ...
        <>
            ...
            >
                <Form layout="vertical" hideRequiredMark
                    ...
                    form={form}
                    initialValues={{ name : name, prefix : prefix}}
                >
                    <Row gutter={16}>
                        <Col span={24}>
                            <Form.Item
