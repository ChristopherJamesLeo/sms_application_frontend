...
const AddDay = ({ fetchData }) => {
    ...
    const formHandler = async (values) => {
        try {
            const response = await api.post('/days', values, {
    ...
    return (
        <>
            <ConfigProvider theme={{ token: { colorPrimary: '#1677ff' } }}>
                <Button type="primary" onClick={() => setOpen(true)}>Add Day</Button>
            </ConfigProvider>
            {contextHolder}
            <Modal
                title="Add Day"
...
export function EditDay({ idx, name, fetchData }) {
    const [open, setOpen] = useState(false);
    const [form] = Form.useForm();
    const [messageApi, contextHolder] = message.useMessage();
    ...
    const formHandler = async (values) => {
        values.id = idx;
        try {
            const response = await api.put(`/days/${idx}`, values, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('api_token')}` }
            });
            if (response.data) {
                if (response.data) {
