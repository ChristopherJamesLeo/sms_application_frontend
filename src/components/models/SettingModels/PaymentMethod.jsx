...
const AddPaymentMethod = ({ fetchData }) => {
    ...
    const formHandler = async (values) => {
        try {
            const response = await api.post('/paymentmethods', values, {
    ...
    return (
        <>
            <ConfigProvider theme={{ token: { colorPrimary: '#1677ff' } }}>
                <Button type="primary" onClick={() => setOpen(true)}>Add Payment Method</Button>
            </ConfigProvider>
            {contextHolder}
            <Modal
...
export function EditPaymentMethod({ idx, name, fetchData }) {
    const [open, setOpen] = useState(false);
    const [form] = Form.useForm();
    const [messageApi, contextHolder] = message.useMessage();
    ...
    const formHandler = async (values) => {
        values.id = idx;
        try {
            const response = await api.put(`/paymentmethods/${idx}`, values, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('api_token')}` }
            });
            if (response.data) {
                if (response.data) {
